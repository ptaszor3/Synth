...
namespace effects {
	double UnsynchronizedTremolo::callback(Note note, Instrument *instrument, DoubleSeconds duration_from_start, int effects_position) { 
		...
		return sample_to_change - sample_to_change * shape->callback(Note(frequency, duration_from_start - note.begin_time, note.end_time, 1), duration_from_start) * max_volume_deviation;
	}
